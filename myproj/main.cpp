...
enum MENU { MENU_CATMULLCLARK, MENU_DRAWWIREFRAME, MENU_EXIT, MENU_DRAWMESH, MENU_LOOP, MENU_DRAWMESHVERTICES,
	...
	MENU_TRIANGULATE, MENU_UNDO, MENU_WRITE, MENU_SIMPLIFY, MENU_SIMPLIFY_10, MENU_SIMPLIFY_20, MENU_SIMPLIFY_30, MENU_DRAWNORMALS, MENU_OPENFILE,
	MENU_TEST_1, MENU_TEST_2, MENU_TEST_3, MENU_TEST_4, MENU_TEST_5
};
...
void menu(int item)
{
	...
	case MENU_SIMPLIFY_10:
	{
		m->collapse(10);
		buff = true;
		break;
	}
	case MENU_SIMPLIFY_20:
	{
		m->collapse(20);
		buff = true;
		break;
	}
	case MENU_SIMPLIFY_30:
	{
		m->collapse(30);
		buff = true;
		break;
...
void display() 
{
	...
	if (drawsilhouette)
	{
		...
		for (vector<myHalfedge *>::iterator it = m->halfedges.begin(); it != m->halfedges.end(); it++)
		{
			...
			auto xn1 = *e->adjacent_face->normal;
			auto xn2 = *e->twin->adjacent_face->normal;
			auto xq = camera_eye - *v1->point;
			xq.normalize();

			const double cp1 = xq * xn1;
			const double cp2 = xq * xn2;
